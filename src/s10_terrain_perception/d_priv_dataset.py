...
class DPrivRecorder:
    def __init__(self, *, teacher_model, metadata=None, teacher_source="privileged_mujoco"):
        ...
        self.rows = {name: [] for name in (
            "student_obs", "teacher_action_raw", "command_raw", "waypoint_id",
            "terrain_id", "episode_id", "step_id", "base_pose_wxyz", "privileged_hit_fraction")}

    def append(self, *, student_obs, teacher_action_raw, command_raw, waypoint_id,
               terrain_id, episode_id, step_id, base_pose_wxyz, privileged_hit_fraction):
        obs = np.asarray(student_obs, np.float32).reshape(441)
        action = np.asarray(teacher_action_raw, np.float32).reshape(16)
        ...
        self.rows["teacher_action_raw"].append(action)
    ...
    def write(self, path):
        ...
        np.savez_compressed(
            ...
            teacher_action_raw=np.asarray(self.rows["teacher_action_raw"], np.float32),
...
def validate_d_priv(path):
    with np.load(path, allow_pickle=False) as data:
        ...
        expected = {
            "student_obs": (count, 441), "teacher_action_raw": (count, 16),
